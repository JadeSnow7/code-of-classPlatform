"""Legacy fallback implementation for tools and GraphRAG index endpoints."""
...
async def add_to_index(req: IndexDocumentRequest) -> IndexDocumentResponse:
    ...
    try:
        ...
        return IndexDocumentResponse(
            ...
            message="Document {0} indexed with {1} chunks".format(req.doc_id, chunks),
        )
    except Exception as exc:  # noqa: BLE001
        return IndexDocumentResponse(
            success=False,
            chunks_affected=0,
            message="Indexing failed: {0}".format(exc),
        )
...
async def delete_from_index(req: DeleteDocumentRequest) -> IndexDocumentResponse:
    ...
    try:
        ...
        return IndexDocumentResponse(
            success=True,
            chunks_affected=chunks,
            message="Document {0} removed, {1} chunks deleted".format(req.doc_id, chunks),
        )
    except Exception as exc:  # noqa: BLE001
        return IndexDocumentResponse(
            success=False,
            chunks_affected=0,
            message="Deletion failed: {0}".format(exc),
        )
