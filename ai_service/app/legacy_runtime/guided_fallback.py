"""Legacy fallback implementation for guided endpoint."""
        ...
        for step_data in data.get("steps", []):
            steps.append(
                LearningStep(
                    ...
                    prerequisite_concepts=step_data.get("prerequisite_concepts", []),
                    requires_tool_verification=step_data.get("requires_tool_verification", False),
                )
            )
...
async def chat_guided(
    ...
    if not session:
        topic = req.topic or (req.messages[-1].content if req.messages else "电磁场学习")
        ...
        path_messages = [
            ...
            {"role": "user", "content": "请为以下学习主题生成学习路径：{0}".format(topic)},
        ]
        try:
    ...
    learning_path_dicts = [
        ...
        for step in session.learning_path
    ]
    ...
    messages.extend([message.model_dump() for message in req.messages])
    ...
    session.messages.extend([message.model_dump() for message in req.messages])
    session.messages.append({"role": "assistant", "content": reply})

    positive_indicators = ["正确", "很好", "太棒了", "完全正确", "进入下一步", "接下来"]
    if any(indicator in reply for indicator in positive_indicators):
